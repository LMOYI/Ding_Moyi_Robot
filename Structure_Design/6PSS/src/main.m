...
A_d = 100;          % Distance btw pts on each line of the Base
B_L = 350;%B_d*ratio;          % Distance btw lines of the Platform
B_d = 80;          % Distance bte pts on each line of the Platform
Hy = 100;              % Initial height [mm]
Da = 500;
Db = 400;
PA = [245.9462  264.8991  180.0331  145.3798  486.4783  382.5314];
A_d = PA(1);
B_L = PA(2);
B_d = PA(3);
Hy = PA(4);
...
% X = [A_d, A_L, B_d, B_L]
Best_Pos = [-26.9721   19.0565  -32.0431   19.2060  344.5381  263.4278];
Best_Pos = [-21.809030337785096,10,0,30,300,2.411431289505870e+02]; % 同向最优 H 150
% Best_Pos = [  -18.2145   13.9076  -22.1539   10.6741  300.0000  200.0000];% 反向结果
Best_Pos = [-10.5190   22.7012  -19.3818   12.8390  300.0000  247.6353]; % 反向 H 120
Best_Pos = [0   30.0000  -21.1486   10.0000  300.0000  265.7420]; % 同向 H 120
...
p0 = [0,-Hy,0]'/n;
type = -3;
[a,b]=Initial_Config(X,type,n,Da,Db);
...
% 检测构型是否正确 - 杆长是否为定值
R_r = rotz(0,'deg');
s_r = ikine(a,b,Len,p0,R_r)
a_r = a;
a_r(2,:) = s_r;
Len_r = vecnorm(repmat(p0,1,6) + R_r*b - a_r ,2,1);
% for i = 1:6
%     Len_r(i) = norm(R_r*b(:,i) + p0 - a_r(:,i));
% end
% p0 = [-10,0,20]'/n;
% R0 =rotz(3,'deg')*roty(3,'deg')*rotx(10,'deg');
% s_r = ikine(a,b,Len,p0,R0)
robot_plot(a, b, Len, p0, R_r)

% 计算力
F = [0;-1;0];
T = [0;0;0];
[f,~] = force(a,b,s_r,p0,R_r,F,T)
% force_x(i,:) = f(1,:);
% force_y(i,:) = f(2,:);
% force_z(i,:) = f(3,:);
Jac = iJacobian(a, b, Len, p0, R_r);
f_ref = Jac'*[F;T]
...
k = 0;
   ...
   % 获取全局最优解
   index = find(params(:,2) == 0);
   % params x change, y change, z change ...
   % x = [all x, then other change]
   % z = [1 for all x, ]
   LTI_location = reshape(fitness3(index),nx*nz,nrx*nry*nrz);
   LTI = min(LTI_location,[],2);
   LTI_value = reshape(LTI,nx,nz);
   [LTIx,LTIz] = meshgrid(x_values,z_values);
   contour(LTIx,LTIz,LTI_value')
   xlabel('x')
   ylabel('y')
   title('最小LTI')
   %%
...
x_values = -25:1:25;
y_values = -Hy;
z_values = -25:1:25;  % 注意：这个范围实际只会生成25
ry_values = -3:1:3;
rx_values = -3:1:3;
rz_values = -30:5:30;
nx= length(x_values);
...
parfor i = 1:num_params
    ...
    pk = [x; y; z]/n;
    Rk = rotz(rz, "deg") * roty(ry, "deg") * rotx(rx, "deg");
    s = ikine(a,b,Len,pk,Rk);
    S_length(i,:) = s;
    % 计算雅可比矩阵及其条件数
    [f,~] = force(a,b,s,pk,Rk,F,T);
    % force_x(i,:) = f(1,:);
    % force_y(i,:) = f(2,:);
    % force_z(i,:) = f(3,:);
    Jac = iJacobian(a, b, Len, pk, Rk);
...
N_all = reshape(fitness_values,nx*ny*nz,nrx*nry*nrz);
N = max(N_all,[],2);
% 
% count = 0;
% for x= -25:1:25
...
%%
clc
F1 = reshape(F_joint(:,1,:),3,num_params);
F2 = reshape(F_joint(:,2,:),3,num_params);
F3 = reshape(F_joint(:,3,:),3,num_params);
F4 = reshape(F_joint(:,4,:),3,num_params);
F5 = reshape(F_joint(:,5,:),3,num_params);
F6 = reshape(F_joint(:,6,:),3,num_params);
...
S = (max(S_length,[],1) - min(S_length,[],1))*n
S_1 = max(S_length,[],1)*n
S_2 = min(S_length,[],1)*n
%%
disp('关节1行程范围')
disp(max(vecnorm(F1)))
disp('关节2最大受力')
