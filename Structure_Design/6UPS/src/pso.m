...
function fitness=fun(X)
    ...
    Ak = Robot.A;
    Bk = Robot.C;
    a = [Ak{1},Ak{2},Ak{3},Ak{4},Ak{5},Ak{6}];
    b = [Bk{1},Bk{2},Bk{3},Bk{4},Bk{5},Bk{6}];

    % 定义参数范围 - 工作空间
    x_values = -5:2:5;
    y_values = -5:2:5;
    z_values = (-10:2:10)+Robot.H;  
    rx_values = -10:2:10;
    ry_values = -10:2:10;
    rz_values = -10:2:10;
    ...
    parfor i = 1:num_params
        ...
        gk = [Rk,pk;0,0,0,1];
        sk = Robot.ikine(gk);%  ikine(a,b,Len,pk,Rk); % 计算行程
        % s_value(i,:) = sk;
        if ~isreal(sk)
            disp([x,y,z,rx,ry,rz])
            disp(X)
